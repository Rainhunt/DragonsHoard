...
export default function LeftNav() {
    ...
    const buttons = useMemo(() => {
        const { pass: dropDown, reject: navBar } = partitionArray([
            ...
            <Button key="my-games" text="My Games" className="nav-button" onClick={() => navigate(ROUTES.MYGAMES)} />,
            <Button key="table" text="Last Game" className="nav-button" onClick={() => window.location.href = import.meta.env.MODE === "production" ? "/DragonsHoard/table/" : "/table/"} />,
        ], () => {
            switch (breakpoint) {
                case "wideScreen":
                    return false;
